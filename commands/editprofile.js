...
function findImageByName(name) {
    ...
    if (foundCharacter?.image) return foundCharacter.image;

    const foundSkin = skins.find(
        s => s.name.toLowerCase() === name.toLowerCase()
    );

    if (foundSkin?.image) return foundSkin.image;
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (main1) {
            ...
            profile.mainGame.mainCharacterImage = findImageByName(main1);
        ...
        if (skin) {
            ...
            if (skinImage) {
                profile.mainGame.mainCharacterImage = skinImage;
            }
